...
export default function About({
  ...
  return (
    <>
      ...
      <div>
        <h3>Meet the team</h3>
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 p-4 md:p-0">
          {people.map(({ slug, frontmatter, htmlContent }) => (
            <div
              key={slug}
              className="border border-gray-200 dark:border-gray-800 m-2 rounded-2xl shadow-lg overflow-hidden flex flex-col text-gray-500 transition duration-75 dark:text-gray-400 group-hover:text-gray-900 dark:group-hover:text-white"
            >
              <CustomImage
                width={240}
                height={240}
              ...
              <p className="p-4 pt-0">{frontmatter.bio}</p>
            </div>
          ))}
        </div>
      </div>
